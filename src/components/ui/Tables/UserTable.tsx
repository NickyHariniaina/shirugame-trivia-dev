        ...
        <TableBody>
          {users.map((user) => (
            <TableRow key={user.id} onClick={() => router.push(`/user/${user.id}`)}>
              ...
              <TableCell className="flex flex-row gap-2 items-center"><Avatar src={user.image || ""} alt={user.username || ""} size={20} /><span className="text-sm">{user.username || user.email}</span></TableCell>
